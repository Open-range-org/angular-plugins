...
export class AngularMultiSelect implements OnInit, ControlValueAccessor, OnChanges, Validator, AfterViewChecked, OnDestroy {
    ...
    @ContentChild(Item, { static: false }) itemTempl: Item | undefined;
    @ContentChild(Badge, { static: false }) badgeTempl: Badge | undefined;
    @ContentChild(Search, { static: false }) searchTempl: Search | undefined;


    @ViewChild('searchInput', { static: false }) searchInput: ElementRef | undefined;
    @ViewChild('selectedList', { static: false }) selectedListElem: ElementRef | undefined;
    @ViewChild('dropdownList', { static: false }) dropdownListElem: ElementRef | undefined;
    @ViewChild('cuppaDropdown', { static: false }) cuppaDropdown: ElementRef | undefined;
    ...
    public selectedItems: Array<any> | undefined;
    public isActive: boolean = false;
    public isSelectAll: boolean = false;
    public isFilterSelectAll: boolean = false;
    public isInfiniteFilterSelectAll: boolean = false;
    public groupedData: Array<any> | undefined;
    filter: any;
    public chunkArray: any[] | undefined;
    ...
    onItemClick(item: any, index: number, evt: Event) {
        ...
        let limit = (this.selectedItems?.length??0) < (this.settings?.limitSelection??0) ? true : false;
        ...
        if (this.isSelectAll || this.data.length > (this.selectedItems?.length??0)) {
            this.isSelectAll = false;
        }

        if (this.data.length == (this.selectedItems?.length??0)) {
    ...
    addSelected(item: any) {
        ...
        if (this.settings.singleSelection) {
            ...
            this.selectedItems = [...this.selectedItems as any, item];
    ...
    public filterInfiniteList(evt: any) {
        ...
        if ((evt != null || evt != '') && !this.settings.groupBy) {
            if ((this.settings?.searchBy?.length??0) > 0) {
              let temp= this.settings.searchBy??[];
                for (let t = 0; t < (this.settings?.searchBy?.length??0); t++) {

                    this.virtualdata.filter((el: any) => {
                        if (el[temp[t].toString()].toString().toLowerCase().indexOf(evt.toString().toLowerCase()) >= 0) {
    ...
    calculateDropdownDirection() {
        ...
        if (this.settings.autoPosition) {
            const dropdownHeight = this.defaultSettings.maxHeight??0;
    ...
    openTowardsTop(value: boolean) {
        ...
        if (value && this.selectedListElem?.nativeElement.clientHeight) {
            ...
            if(this.settings.tagToBody){
                this.dropDownTop = (elem.getBoundingClientRect().y - this.selectedListElem.nativeElement.clientHeight*2 - 15 - (this.defaultSettings.maxHeight??0))+'px';
